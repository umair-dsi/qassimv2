/**
Originaly https://github.com/jonathandarrer that was targetted for leaflet 0.6.x
modified by imran to cater to leaflet 1.x.x
added _spliceLatLngs as this was removed in v 1.x.x
added newDrawing flag
**/
L.Control.Measure = L.Control.extend({
    ...
    _mouseMove: function (e) {
        ...
        if (!this._layerPaintPathTemp) {
            ...
            this._spliceLatLngs(0, 2, [this._lastPoint, e.latlng], this._layerPaintPathTemp);
        }
    ...
    _spliceLatLngs: function (index, count, toAdd, _layer) {
        var latLngs;
        if (this.options.mode == 'length') {
            latLngs = _layer.getLatLngs();
        }
        else {
            latLngs = _layer.getLatLngs();
        }
    ...
    _finishPath: function () {
        ...
        if (this._layerPaintPath != null) {
            if (this.options.mode != 'length' && this._layerPaintPath.getLatLngs()[0].length > 2) {
                var area = L.GeometryUtil.geodesicArea(this._layerPaintPath.getLatLngs()[0]);
                var lastPointPolygon = this._layerPaintPath.getLatLngs()[0][0];
