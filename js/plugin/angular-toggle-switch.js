angular.module('toggle-switch', ['ng']).directive('toggleSwitch', ['$compile','$rootScope','$translate', 'utilService', function($compile,$rootScope,$translate,utilService) {
	return {
		restrict: 'EA',
		replace: true,
		require:'ngModel',
		...
		template:
					'<div class="ats-switch" ng-keypress="onKeyPress($event)" ng-class="{ \'disabled\': isDisabled }" role="switch" aria-checked="{{!!model}}">' +
						'<div class="switch-animate" ng-class="{\'switch-off\': !model, \'switch-on\': model}" style="height:30px;">' +
							'<span class="switch-left" ng-style="(language==\'en-US\') && {} || {\'padding-top\': \'9px\'}"></span>' +
		...
		compile: function(element, attrs) {
			if (angular.isUndefined(attrs.onLabel)) {
				attrs.onLabel = ' العربية ';
			}
			if (angular.isUndefined(attrs.offLabel)) {
				attrs.offLabel = '  English  ';
			}
			...
			return function postLink(scope, iElement, iAttrs, ngModel) {
				...
				scope.$watch('isEnglish', function(newValue) {
					if (!scope.isDisabled) {
						if(newValue == "en-US") {
							scope.model = false;
						} else {
